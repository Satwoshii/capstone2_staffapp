...
; ============================================================
; SYSWATCH ADMIN / ITSO / TEACHER INSTALLER v2.9.8
; Includes Teacher role, Audit Log name/email display, export tools,
; automatic server discovery, repairs, maintenance, reports and chats.
;
; Put this file inside:
;   <admin-project>\installer\Syswatch_Admin_Installer_v2.9.8.iss
;
...
#define VCRedistPath AddBackslash(SourcePath) + VCRedistFile
  ...
  #error "lib\main.dart was not found. Check that this is the Syswatch Admin project."
#endif

#if !FileExists(AddBackslash(ProjectRoot) + "lib\screens\staff_login_screen.dart")
  #error "staff_login_screen.dart was not found. Copy the latest Admin files first."
#endif
  ...
  #error "last_known_user_screen.dart was not found. Copy the v2.9.8 Audit Log files first."
#endif

#if !FileExists(AddBackslash(ProjectRoot) + "lib\screens\reports_screen.dart")
  #error "reports_screen.dart was not found. Copy the latest Reports files first."
#endif

#if !FileExists(AddBackslash(ProjectRoot) + "lib\screens\repair_management_screen.dart")
  #error "repair_management_screen.dart was not found. Copy the latest Repair files first."
#endif

#if !FileExists(AddBackslash(ProjectRoot) + "lib\screens\lab_maintenance_overview_screen.dart")
  #error "lab_maintenance_overview_screen.dart was not found. Copy the latest Maintenance files first."
#endif

#if !FileExists(AddBackslash(ProjectRoot) + "lib\screens\preventive_maintenance_screen.dart")
  #error "preventive_maintenance_screen.dart was not found. Copy the latest Preventive Maintenance files first."
#endif

#if !FileExists(AddBackslash(ProjectRoot) + "lib\screens\support_chat_screen.dart")
  #error "support_chat_screen.dart was not found. Copy the latest ITSO Support files first."
#endif
  ...
  #error "admin_teacher_chat_screen.dart was not found. Copy the latest Admin Teacher Chat files first."
#endif
  ...
  #error "export_service.dart was not found. Copy the latest export feature files first."
#endif
...
[Setup]
; Keep this AppId unchanged so newer installers upgrade the existing Admin app.
...
[UninstallDelete]
; Remove only installed program files. Admin preferences/session data stored outside
; Program Files is intentionally preserved unless your app removes it separately.
